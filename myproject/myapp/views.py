...
def buyshare(request):
    ...
    if request.method=="POST":
        ...
        if Portfolio.objects.filter(userdetails=userdetails).filter(stock_name=stock_name).exists():
            portfolio=Portfolio.objects.filter(userdetails=userdetails).filter(stock_name=stock_name)[0]
            ...
            new_portfolio=Portfolio(userdetails=userdetails,stock_name=stock_name,no_of_share=no_of_shares,stock_cc=stock_cc)
            new_portfolio.save()
...
def sellshare(request):
    ...
    if request.method=="POST":
        ...
        if  Portfolio.objects.filter(userdetails=userdetails).filter(stock_name=stock_name).exists():
            ...
            portfolio.total_profit=portfolio.total_profit+(no_of_shares*sell_price-no_of_shares*(portfolio.stock_cc))
            if((portfolio.no_of_share)==no_of_shares):
...
def register(request):
    ...
    if request.method=="POST":
        ...
        user1=User.objects.create_user(username,'sameerpanda.2019',password)
        user1.save()
